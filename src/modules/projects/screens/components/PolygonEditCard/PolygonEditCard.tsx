...
export const PolygonEditCard: React.FC<EditableCardProps> = ({
  ...
  return (
    <div className="card-container bg-white shadow-md rounded p-4 m-4">
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-600">Sides:</label>
        <input
          type="number"
          value={editingSides}
      ...
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-600">Vertices:</label>
        {editingVertices.map((vertex, index) => (
          <input
            key={vertex.id}
            type="number"
            value={vertex.x}
            onChange={(e) => handleVerticesChange(index, parseInt(e.target.value, 10))}
